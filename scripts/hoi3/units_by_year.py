...
for year in range(1936, 1948):
    ...
    for unit_type, unit_file in files.items():
        unit_file.write("== %d ==\n" % year)
        unit_file.write(pyradox.table.make_table(units, 'wiki', base_columns[unit_type], lambda k, v: v["type"] == unit_type and v["active"]))
        unit_file.write("=== Derived statistics ===\n")
        unit_file.write(pyradox.table.make_table(units, 'wiki', derived_columns[unit_type], lambda k, v: v["type"] == unit_type and v["active"]))
